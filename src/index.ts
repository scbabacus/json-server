...
import { reloadConfig, config, reloadServiceFile, loadLibraries } from "./utils";
...
const con = new log.transports.Console({format: log.format.printf((info) => `${moment().toISOString()} [${info.level}] ${info.message}`)});
...
function processService(services: Service) {
	for (let svc of Object.keys(services)) {
		for (let method of Object.keys(services[svc])) {
			const service = services[svc];

			switch (method.toLowerCase()) {
				case "get": app.get(svc, getServiceHandler(service[method])); break;
				case "post": app.post(svc, getServiceHandler(service[method])); break;
				case "put": app.put(svc, getServiceHandler(service[method])); break;
				case "delete": app.delete(svc, getServiceHandler(service[method])); break;
				case "patch": app.patch(svc, getServiceHandler(service[method])); break;
				case "options": app.options(svc, getServiceHandler(service[method])); break;
				case "head": app.head(svc, getServiceHandler(service[method])); break;
				case "*": app.all(svc, getServiceHandler(service[method])); break;
				default: 
...
function processCommand(command: string, innerJson: any, context: Object): any {
	if (command === "$array") return processArrayCommand(innerJson, context);
